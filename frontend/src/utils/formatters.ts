...
export function formatScore(score: OverallScore | OverallAssessment | null | undefined): string {
  ...
  if (Number.isFinite(numeric)) return numeric < 0 ? "--" : numeric.toFixed(numeric % 1 === 0 ? 0 : 1);
  return String(raw);
}
...
export function getOverallScore(response: FinancialAnalysisResponse | null): OverallScore | OverallAssessment | null {
  return response?.report_result?.overall_assessment || response?.analysis_result?.overall_score || null;
}

export function getSummary(response: FinancialAnalysisResponse | null): string {
  return compactText(
    response?.report_result?.executive_summary || response?.analysis_result?.summary || response?.assistant_message,
    "分析已完成，请查看下方完整报告。",
  );
}

export function getReportMarkdown(response: FinancialAnalysisResponse | null): string {
  return compactText(response?.final_report || response?.report_result?.markdown_report, "");
}

export function collectMetrics(response: FinancialAnalysisResponse | null): SupportingMetric[] {
  const reportMetrics = response?.report_result?.sections?.flatMap((section) => section.supporting_metrics || []) || [];
  const analysisMetrics = response?.analysis_result?.dimensions?.flatMap((dimension) => dimension.supporting_metrics || []) || [];
  const unique = new Map<string, SupportingMetric>();
  for (const metric of [...reportMetrics, ...analysisMetrics].filter((item) => item.name)) {
